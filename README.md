...
> **v0.5.18 起扩展 ID 已固定**（manifest 内置公钥，ID = `mkgiaknigibmeplddicmcflhdmacajgn`）：从此**解压到任何路径、任何文件夹名，配置都不再丢失**，升级可随意替换/换路径。仅"从 0.5.17 及更早版本升级"这一次 ID 会变化（浏览器视为新扩展、设置清零）——先在旧版设置页底部「配置备份」导出一份，装好新版后导入即全部恢复，此后不再需要。
